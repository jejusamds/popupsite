...
class Gallery_model extends CI_Model {
  ...
  public function get_gallery($row_type = 'row', $select = '*', $gbbs_id = '', $gbbs_type = '') {
    ...
    if($select == '*'){
      $sql = "SELECT G.*, U.usr_name FROM geumsa_gallery G ";
    } else {
      $sql = "SELECT $select FROM geumsa_gallery G ";
    }
  ...
  function set_gallery($gbbs_id, $gbbs_type, $gbbs_title, $gbbs_contents, $gbbs_img, $usr_id) {
    ...
    if(empty($gbbs_id)){
      ...
      return $this->db->insert('geumsa_gallery');
    } else {
      ...
      $this->db->set('gbbs_img', $gbbs_img);
      $this->db->set('usr_id', $usr_id);
      return $this->db->update('geumsa_gallery');
    }
  }
  ...
  public function get_count($gbbs_type, $type, $value){
    ...
    return $this->db->count_all_results("geumsa_gallery");
  }

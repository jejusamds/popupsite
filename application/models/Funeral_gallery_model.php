...
/* 
* User Model
* @functions  get_funeral_gallery
* @functions  set_funeral_gallery
* @functions  paging
*/
class Funeral_gallery_model extends CI_Model {

  function  __construct() {
    parent::__construct();
  }

  public function recent_funeral_gallery(){

    $sql = "SELECT
              gbbs_title TITLE,
              gbbs_img       IMG_PATH,
              IF(DATE_ADD(created, INTERVAL +1 DAY) > NOW(), 'Y', 'N') IS_NEW
            FROM geumsa_funeral_gallery
            WHERE is_delete = 'N'
  ...
  public function get_funeral_gallery($row_type = 'row', $select = '*', $gbbs_id = '', $gbbs_type = '') {

    if($select == '*'){
      $sql = "SELECT G.*, U.usr_name FROM geumsa_funeral_gallery G ";
    } else {
      $sql = "SELECT $select FROM geumsa_funeral_gallery G ";
    }
  ...
  function set_funeral_gallery($gbbs_id, $gbbs_type, $gbbs_title, $gbbs_contents, $gbbs_img, $usr_id) {
    ...
    if(empty($gbbs_id)){
      ...
      return $this->db->insert('geumsa_funeral_gallery');
    } else {
      ...
      $this->db->set('gbbs_img', $gbbs_img);
      $this->db->set('usr_id', $usr_id);
      return $this->db->update('geumsa_funeral_gallery');
    }
  }

  public function get_count($gbbs_type, $type, $value){

    $numrows = 0;
    $this->db->where("is_delete", "N");

    if(!empty($gbbs_type)){
      $this->db->where('gbbs_type', $gbbs_type);
    }
    ...
    return $this->db->count_all_results("geumsa_funeral_gallery");
  }

  function delete_funeral_gallery($id = array()){
    if(is_array($id)){
  ...
  public function paging($offset = 0, $per_page, $is_admin = 'Y', $gbbs_type, $type, $value){
    ...
    if($type != '' && $value != ''){
      $sql .= " AND G.$type = ".$this->db->escape((String) $value);
    }

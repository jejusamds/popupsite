  <!-- 위치 -->
  <section class="sub_sitemap" id="sub_sitemap">
    <div class="container">
      <div class="row mt-2 mt-lg-5">
        <div class="col-12">
         <img src="img/sub_sitemap_home.png"> &nbsp > &nbsp 
         <a href="#">병원소식</a> &nbsp > &nbsp  
         공지사항
        </div>
      </div>
    </div>
  </section>
  <!-- 위치 -->

  <!-- 타이틀 -->
  <section class="sub_title">
    <div class="container">
      <div class="row mt-2 mt-lg-5">
        <div class="col-12 justify-content-center text-center">
          <p class="sub_title_big mb-lg-2 mb-1 mt-3">공지사항</p>
        </div>
      </div>
    </div>
  </section>
  <!-- //타이틀 -->

  <!-- 서브 탑 이미지 -->
  <section id="sub05_topimg" class="mt-lg-3 mt-2">
      <h2 class="col-7 ml-lg-5 sub_topimg_text text-center shaow wow animated fadeInDown" data-wow-duration="0.2s">
        “ 의료법인<span>해림의료재단</span><br>부산금사요양병원을<br>찾아주셔서 감사합니다. "
      </h2>

  </section>
  <!-- //서브 탑 이미지 -->



  
  <!-- 게시판 상세페이지 -->
  <section id="borad_detale" class="mt-lg-5 mt-4">
    <div class="container">
      <div class="row board_detail_top mt-5">
        <div class="col-2 col-lg-1 pr-0">
          <h6> 제목 </h6>
        </div>
        <div class="col-10 col-lg-11">
          <h4 class="mb-0"> {BBS_TITLE} </h4>
        </div>
        <div class="col-12 p-0">
          <hr>
        </div>
        <div class="col-2 col-lg-1 pr-0">
          <h6> 작성 </h6>
        </div>
        <div class="col-7 col-sm-6 col-lg-7">
          <h6> {USR_NAME} </h6>
        </div>
        <div class="col-12 col-sm-4 text-right">
          <h6> 작성일 {DATE} </h6>
        </div>
      </div>
      <div class="row board-text mt-3 ">
        <div class="col-12">
          {BBS_CONTENTS}
        </div>
      </div>
      <div class="row board_detail_bottom mt-5 mb-3">
        {PREV}
        {NEXT}
      </div>
	 
	  <a href="{BASE_URL}index.php/news/notice">
      <div class="row mb-5">
		<div class="col-12">
          <h6 class="btn board_btn_list">목록</h6>
        </div>	
      </div>
	  </a>
    </div>
  </section>
  <!-- //면회안내 -->

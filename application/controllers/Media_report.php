...
class Media_report extends CI_Controller {
  ...
  public function manage_index($type = '-', $value = '-', $offset = '0') {
    ...
    $pagination_config['base_url'] = site_url('manager/media_report/index/'.$type.'/'.$value);

    if($type  == '-')          $type = '';
    if($value == '-')          $value = '';

    $data    = $this->board_model->paging($offset, $per_page, 'Y', 'media_report', $type, $value);
    $rows    = $data['rows'];
    ...
    $this->parser->parse("media_report/index", $tpl_vars);
  ...
  public function manage_write($bbs_id = ""){ 
    ...
    if(!empty($_POST)){
      ...
      if(empty($tpl_vars['ERROR'])) {
        $result = $this->board_model->set_board($bbs_id, "media_report", $title, $contents, L_USR_ID);
        if($result){
          $tpl_vars['REDIRECT_URL'] = base_url()."index.php/manager/media_report/index";
      ...
      if(!empty($bbs_id)){
        $rows = $this->board_model->get_board('row', '*', $bbs_id, 'media_report');
        $title    = $rows['bbs_title'];
        $contents = $rows['bbs_contents'];
      }
    }

    $tpl_vars['BBS_ID']       = $bbs_id;
    $tpl_vars['BBS_TITLE']    = $title;
    $tpl_vars['BBS_CONTENTS'] = $contents;

    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("media_report/write", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);    
  }
  ...
  function manage_view($bbs_id){
    ...
    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("media_report/view", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);
  }
  ...
  public function request_index($type = '-', $value = '-', $offset = '0') {
    ...
    $pagination_config['base_url'] = site_url('media_report/request_index/'.$type.'/'.$value);
    ...
    $this->parser->parse("media_report/request_index", $tpl_vars);
  }
  ...
  function request_view($bbs_id = ""){
    ...
    $this->parser->parse("media_report/request_view", $tpl_vars);
  }

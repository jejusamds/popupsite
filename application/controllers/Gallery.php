...
class Gallery extends CI_Controller {
  ...
  public function manage_index($type = '-', $value = '-', $offset = '0') {
    ...
    if (!empty ($_POST['checked'])) {
      $result = $this->gallery_model->delete_($_POST['checked']);
      if($result){
        $tpl_vars['REDIRECT_URL'] = base_url()."index.php/manager/gallery/index";
    ...
    $pagination_config['base_url'] = site_url('/manager/gallery/index/'.$type.'/'.$value);
    ...
    $this->parser->parse("gallery/index", $tpl_vars);
  ...
  public function manage_write($gbbs_id = ""){ 
    ...
    if(!empty($_POST)){
      ...
      if(!empty($gbbs_id)){
        $rows = $this->gallery_model->get_gallery('row', '*', $gbbs_id);
        if(empty($rows)){
          $gbbs_id = "";
        }
      }
      ...
      if(empty($tpl_vars['ERROR'])) {
        $result = $this->gallery_model->set_gallery($gbbs_id, $gbbs_type, $title, $contents, $img_filename, L_USR_ID);
    ...
    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("gallery/write", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);    
  }
  ...
  function manage_view($gbbs_id){
    ...
    if(empty($gbbs_id)){
      ...
      $this->gallery_model->set_views($gbbs_id);
      $rows = $this->gallery_model->get_gallery('row', '*', $gbbs_id, 'gallery');
      $rows = array_change_key_case($rows, CASE_UPPER);
      $tpl_vars = array_merge($rows, $tpl_vars);
    }

    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("gallery/view", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);
  }
  ...
  function manage_request_delete(){
    ...
    if(empty($_POST['gbbs_id'])){
      ...
      $result = $this->gallery_model->delete_gallery($_POST['gbbs_id']);
      if($result){
        $tpl_vars['ERROR'] = "OK";
        $tpl_vars['ERROR_MESSAGE'] = "삭제되었습니다";
        $tpl['CONTENTS'] = json_encode($tpl_vars, TRUE);
  ...
  function request_index(){
    ...
    $select = " G.gbbs_id       ID,
                ...
                G.gbbs_img      IMG_NAME
              ";
    $rows = $this->gallery_model->get_gallery('result', $select);
    $category = $this->gallery_model->get_category();
    ...
    $this->parser->parse("gallery/request_index", $tpl_vars);
  }
}

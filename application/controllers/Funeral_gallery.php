...
class Funeral_gallery extends CI_Controller {
  ...
  public function manage_index($type = '-', $value = '-', $offset = '0') {
    ...
    if (!empty ($_POST['checked'])) {
      $result = $this->funeral_gallery_model->delete_($_POST['checked']);
      if($result){
        $tpl_vars['REDIRECT_URL'] = base_url()."index.php/manager/funeral_gallery/index";
    ...
    $pagination_config['base_url'] = site_url('/manager/funeral_gallery/index/'.$type.'/'.$value);
    ...
    $this->parser->parse("funeral_gallery/index", $tpl_vars);
  ...
  public function manage_write($gbbs_id = ""){ 
    ...
    if(!empty($_POST)){
      ...
      if(!empty($gbbs_id)){
        $rows = $this->funeral_gallery_model->get_funeral_gallery('row', '*', $gbbs_id);
        if(empty($rows)){
          $gbbs_id = "";
        }
      }
...
//        $tpl_vars['ERROR_MESSAGE'] = "메모를 입력해주세요.";
      ...
      if(empty($tpl_vars['ERROR'])) {
        $result = $this->funeral_gallery_model->set_funeral_gallery($gbbs_id, $gbbs_type, $title, $contents, $img_filename, L_USR_ID);

        if($result){          

          @copy(DATA_FILEPATH."/temp/".$img_filename, DATA_FILEPATH."/funeral_gallery/".$img_filename);
...
//    $tpl_vars['GBBS_CONTENTS']   = $contents;
    ...
    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("funeral_gallery/write", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);    
  }
  ...
  function manage_view($gbbs_id){
    ...
    if(empty($gbbs_id)){
      ...
      $this->funeral_gallery_model->set_views($gbbs_id);
      $rows = $this->funeral_gallery_model->get_funeral_gallery('row', '*', $gbbs_id, 'funeral_gallery');
      $rows = array_change_key_case($rows, CASE_UPPER);
      $tpl_vars = array_merge($rows, $tpl_vars);
    }

    $this->parser->parse("layouts/admin/header", $tpl_vars);
    $this->parser->parse("funeral_gallery/view", $tpl_vars);
    $this->parser->parse("layouts/admin/footer", $tpl_vars);
  }
  ...
  function manage_request_delete(){
    ...
    if(empty($_POST['gbbs_id'])){
      ...
      $result = $this->funeral_gallery_model->delete_funeral_gallery($_POST['gbbs_id']);
      if($result){
        $tpl_vars['ERROR'] = "OK";
        $tpl_vars['ERROR_MESSAGE'] = "삭제되었습니다";
        $tpl['CONTENTS'] = json_encode($tpl_vars, TRUE);
  ...
  function request_index(){
    ...
    $select = " G.gbbs_id       ID,
                ...
                G.gbbs_img      IMG_NAME
              ";
    $rows = $this->funeral_gallery_model->get_funeral_gallery('result', $select);
    $category = $this->funeral_gallery_model->get_category();

    if(empty($rows)) {
      $tpl_vars['EMPTY'] = "<div class='col-md-12 col-sm-12 col-xs-12'>".EMPTY_DATA."</div>'";
    } else {
      $tpl_vars['EMPTY'] = "";
      foreach($rows as $k => $v){
        $v['BASE_URL'] = base_url();
    ...
    $this->parser->parse("funeral_gallery/request_index", $tpl_vars);
  }
}
